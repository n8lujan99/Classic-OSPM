...
def build_observables(config):
    return OSPMObservablesStellar.from_star_table( config["DATA_CSV"], inclination_deg=config["INCLINATION_DEG"], Norbit=config["NORBIT"], stellar_model=config.get("STELLAR_MODEL", None), 
                                            surface_brightness_path=config.get("SURFACE_BRIGHTNESS_CSV", None), kinematic_bins_path=config.get("KINEMATIC_BINS_CSV", None), config=config)
def solve_ospm_theta(theta, obs, *, halo_type="nfw"):
    raise RuntimeError( "solve_ospm_theta is disabled in the Karl-style branch. "
