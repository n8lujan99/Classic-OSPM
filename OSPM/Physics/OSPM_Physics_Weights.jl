...
function _print_tracer_iteration_diagnostics(A_light::Matrix{Float64}, w::Vector{Float64}, light_target::Vector{Float64}, light_sigma::Vector{Float64}; iteration::Int, top_n::Int=5)
    ...
    size(A_light, 2) == length(w) || error("Tracer iteration diagnostic weight length mismatch")
    ...
    nprint = min(top_n, length(order))
    Nbase = length(w) ÷ 2

    println("[TRACER ITER SUMMARY]",
        " iteration=", iteration,
    ...
    @inbounds for rank in 1:nprint
        ...
        n_support = 0
        n_weighted_support = 0

        for c in 1:Nbase
            jpro = 2 * c - 1
            jret = 2 * c
            apro = A_light[ib, jpro]
            aret = A_light[ib, jret]

            if apro > 0.0 || aret > 0.0
                n_support += 1
            end

            contribution = apro * w[jpro] + aret * w[jret]
            pair_contrib[c] = contribution
            contribution > 0.0 && (n_weighted_support += 1)
        end
...
function karl_spear_update_expanded(w_all::Vector{Float64}, Norbit::Int, Cm::Matrix{Float64}, target::Vector{Float64}, dS::Vector{Float64}, ddS::Vector{Float64}; apfac::Float64=DEFAULT_KARL_APFAC, entropy_floor::Float64=DEFAULT_KARL_ENTROPY_FLOOR, step_safety::Float64=DEFAULT_KARL_STEP_SAFETY, compute_rcond::Bool=true, rcond_warn::Float64=DEFAULT_KARL_SPEAR_RCOND_WARN, active_bound=nothing, max_active_passes::Int=512, bound_kkt_tol::Float64=1.0e-10)
    ...
    return (
        ...
        active_bound=falses(Norbit), active_set_stabilized=true, active_set_failure_reason=:none, active_passes=1,
