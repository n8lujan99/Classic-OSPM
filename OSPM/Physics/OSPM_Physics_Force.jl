...
@inline function stellar_Phi_karl_light_grid(r::Float64, ML::Float64, grid)
    # Spherical-grid potential approximation.
    # Force uses exact enclosed-mass interpolation from the grid.
    rr = max(r, 1e-30)
    Menc = stellar_Menc_karl_light_grid(rr, ML, grid)
    return -G * Menc / rr
end
...
function build_axisymmetric_force_table(grid, ML::Float64; nR::Int=96, nZ::Int=96, nphi::Int=32)
    ...
    @inbounds for i in 1:nR
        ...
        for j in 1:nZ
            zf = z_axis[j]
            fr, fz = _axisym_force_from_cells(Rf, zf, ML, grid; nphi=nphi)
            FR[i, j] = fr
            FZ[i, j] = fz
        end
    end
    return ( R_axis = R_axis, z_axis = z_axis, FR = FR, FZ = FZ, nR = nR, nZ = nZ, nphi = nphi )
end
...
mass_enclosed_two_radii(rin, rout, rho_s, r_s, MBH, ML, halo_type; stellar_model=nothing, halo_q_axis_ratio=1.0, karl_halo_params=nothing) = begin
    if abs(max(abs(f64(halo_q_axis_ratio)), 1e-6) - 1.0) > 1e-8
        error(
            "mass_enclosed_two_radii is only physically meaningful for spherical halo models. " *
            "For flattened halo_q_axis_ratio, use a force diagnostic instead."
        )
    end

    if is_axisymmetric_stellar_model(stellar_model)
        error(
            "mass_enclosed_two_radii is only physically meaningful for spherical force models. " *
            "For axisymmetric_density_grid, use a force diagnostic instead."
        )
    end
