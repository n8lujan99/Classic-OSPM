...
def _choose_column(fields: list[str], candidates: list[str], label: str) -> str:
    for name in candidates:
        if name in fields:
            return name
    raise KeyError(f"Could not find {label}; tried {candidates}. Available columns: {fields}")
...
def _build_seeing_transfer(grid: KarlProjectedGrid, seeing_arcsec: float, psf_samples: int) -> tuple[np.ndarray, np.ndarray]:
    if not (math.isfinite(seeing_arcsec) and seeing_arcsec > 0.0):
        raise ValueError("seeing_arcsec must be finite and positive")
    if psf_samples < 2:
...
class SurfaceBrightnessProfile:
    def __init__(self, path: Path):
        ...
        r_col = _choose_column(fields, ["R_pc", "R_mid_pc", "r_pc", "radius_pc"], "surface-brightness radius column")
        s_col = _choose_column(fields, ["Sigma", "Sigma_Lsun_pc2", "surface_density", "surface_brightness"], "surface-brightness Sigma column")

        R = np.asarray(data[r_col], dtype=np.float64)
        S = np.asarray(data[s_col], dtype=np.float64)
...
def _read_stars(cfg: dict) -> dict[str, np.ndarray]:
    ...
    for name in (r_col, v_col, verr_col):
        if name not in data:
            raise KeyError(f"{path} is missing required numeric star column {name!r}")

    if "vlos_rel_kms" in data:
        vrel = data["vlos_rel_kms"].copy()
    else:
        vsys = float(_require(cfg, "V_SYS_KMS"))
        vrel = data[v_col] - vsys
...
CSV_FIELDS = [
    ...
    "source_data_csv", "source_surface_brightness_csv",
...
def build_karl_observables(config_path: Path, output_override: Path | None = None) -> Path:
    ...
    if losvd_center_mode != "systemic":
        raise ValueError("Current Karl mode-0 reconstruction supports losvd_center_mode='systemic'")
    if losvd_shape != "gaussian":
        raise ValueError("Current Karl mode-0 reconstruction supports losvd_shape='gaussian'")
    ...
    profile = SurfaceBrightnessProfile(sb_path)
    stars = _read_stars(cfg)
