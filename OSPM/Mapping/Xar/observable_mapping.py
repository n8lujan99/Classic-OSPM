...
def cmd_build_axisymmetric_light_grid(args):
    ...
    if args.radial_model == "shell":
        ...
        if shell is None:
            raise ValueError("shell radial model requires R_inner_pc, R_outer_pc, and light_frac columns")
    elif args.radial_model == "abel":
        shell = abel_grid_from_surface_brightness(sb, args.ltot, radius_col=args.radius_col, sigma_col=args.sigma_col, n_radial=args.n_radial)
    else:
...
def build_parser():
    ...
    p_axis.add_argument("--radial-model", default="abel", choices=["abel", "shell"],
                        help="Use Abel-deprojected radial density or the input profile's native shell light fractions.")
